...
def get_inner_day_price_list(index_lst, company):
	start_time = time.time() - (60*60*24)*10 - 10
	current_time = time.time()
	current_time = time.time()
	conn = psycopg2.connect(
...
def get_tick_price_list(company):
	start_time = time.time() - (60*60*24)*10 - 24000
	current_time = time.time()
	index_lst = int(int((current_time-start_time))/86400)
	current_time = time.time()
	index_tmp = int(current_time-start_time-index_lst*86400)
	...
	if index_tmp <= 36000:
		for index in range(index_tmp):
			if index % (300) == 0 and index != 36000 and index != 0:
				tick_price_graph.append({"time": (index+start_time), "value": round(price_list[index_lst][index], 2)})
	else:
...
def get_all_current_price(company_list):
	...
	company_list_3 = company_list[5:8]
...
def current_all_prices():
	get_all_current_price(["ast", "dsc", "fsin", "hhw", "jky", "sgo", "wrkn", "index"])
	return jsonify(companies_current_prices)
...
def current_prices():
	...
	if index_tmp <= 36000:
		open_price = get_current_price(comp_name, index_lst, 0)
		current_price = get_current_price(comp_name, index_lst, index_tmp)
		change = round(
			(current_price - open_price), 2)
		pct_change = round(
			(current_price - open_price) / open_price * 100, 2)
		current_price_dict = {"price":  float(round(
			current_price, 2)), "change": float(change), "pct_change": float(pct_change)}
		return jsonify(current_price_dict)
	elif index_tmp > 36000 and index_tmp < 86400:
		final_price = get_current_price(comp_name, index_lst, -1)
		current_price_dict = {"price":  round(
...
def tick_graph():
	comp_name = json.loads(request.data)
	tick_price_graph = get_tick_price_list(comp_name)
	return tick_price_graph
...
def day_graph():
	...
	for index in range(index_lst):
		inner_day_lst = get_inner_day_price_list(index, comp_name)
...
def hour_graph():
	...
	comp_name = json.loads(request.data)
	graph_lst = []
	inner_day_lst = get_day_price_list(comp_name)
	for index in range(index_lst):
