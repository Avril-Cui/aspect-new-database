...
class StockSimulator:
	...
	def per_second_price(self, mu, sigma):
		...
		simulated_price = self.second_price + mu * self.second_price * dt+ sigma * np.sqrt(self.second_price) * tmp_bm
		return simulated_price
	...
	def loop_per_second(self):
		...
		for index in range(len(split[:-1])):
			...
			dt = 1/(60*60)
			num = int(9/self.dt) #9 denotes the market opens for 9 hours per day.
			initial_price = one_day[0]

			daily_price = [initial_price]

			for inx in range(num):
				mu_tmp = drift[int(inx*self.dt)]
				sigma = 0.05
