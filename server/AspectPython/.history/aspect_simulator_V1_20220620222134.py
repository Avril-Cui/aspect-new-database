...
def generate_stock_parameter(original_price, original_index_price, target_adjustment, comp_weight, index_weight):
    orginal_price_list = original_price['META'].to_list()
    index_price = original_index_price[:len(orginal_price_list)]

    convolution_df = original_price.rolling(window=10).mean()
    mu_df = convolution_df.pct_change()
    mu = mu_df["META"].to_list()[10:]

    convolution_df_index = index_price.rolling(window=10).mean()
    mu_df_index = convolution_df_index.pct_change()
    mu_index = mu_df_index["^GSPC"].to_list()[10:]

    sigma = []

    sampling_price_list = []
    for index in range(len(orginal_price_list)):
        price_list = []
        if len(orginal_price_list)-index > 10:
            for number in range(10):
                price_list.append(orginal_price_list[index+number])
    ...
    for index in range(len(sampling_price_list)):
        ...
        simulated_price.append(simulated_price[-1]+(mean_company*comp_weight+ index_weight*mean_index)* X[-1] + sigm * dWt)
...
plt.plot(stock_price_comp, color='orange')
plt.figure(figsize = (9,4))
plt.plot(index_price, color='orange')
plt.show()


simulated_price_comp, tick_based_price_list_comp, day_based_price_list_comp, second_based_price_list_comp = generate_stock_parameter(stock_price_comp, index_price, target_adjustment, comp_weight, index_weight)
