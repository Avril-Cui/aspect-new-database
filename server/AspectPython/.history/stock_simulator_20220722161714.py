...
class StockSimulator:
	...
	def per_second_price(self, mu, sigma):
		...
		price_change = mu * self.second_price * dt + sigma * np.sqrt(self.second_price) * tmp_bm
		self.second_price += price_change
	...
	def loop_per_second(self):
		...
		for index in range(len(split[:-1])):
			...
			for inx in range(num):
				...
				# 	print(inx)
				sigma = 0.05
				self.per_second_price(mu_tmp, sigma)
				daily_price.append(self.second_price)
			
			if daily_price[-1]>1090:
				print(daily_price)
				print(initial_price)
				print("end:" + str(one_day[-1]))
