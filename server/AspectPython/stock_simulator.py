...
class StockSimulator:
	...
	def __init__(
		...
		self.original_micro = micro_params
	...
	def loop_per_second(self):
		...
		for index in range(len(split[:-1])):
			...
			for inx in range(num):
				...
				self.micro_params = self.original_micro(self.total_index, self.price_change)
			...
			daily_price[-1] = round(one_day[-1],2)
...
wrkn_initial_price = 55.67
wrkn_macro_params = Wakron_macro["IPO"]()
wrkn_micro_params = Wakron_micro["IPO"]

wrkn_simulator = StockSimulator(wrkn_initial_price, wrkn_macro_params, wrkn_micro_params)
wrkn_second_price_lst = wrkn_simulator.loop_per_second()

print(wrkn_second_price_lst)