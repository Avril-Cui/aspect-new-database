...
macro = index_params["mete1_1"]["macro"]
...
for index in range(len(combinated_price)):
    if index > 0:
        price_generator = StockSimulator(
            1200/combinated_price[1], combinated_price[index], combinated_price, index, micro, index_params["mete1_1"])
...
for index in range(len(final_price_list)):
    random_modifier = np.random.normal(0, 0.01)
    user_interaction_modifier.append(random_modifier)

final_price_list = np.add(final_price_list, user_interaction_modifier)
tick = []
for index in range(len(final_price_list)):
	if index % 30 == 0:
