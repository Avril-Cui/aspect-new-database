...
class WaveModifier:
    ...
    def price_wave_addition(self, transformation, intensity, length, *argv):
        ...
        for wave_info in argv:
            price = mid_price[wave_info["start_point"]:wave_info["start_point"]+wave_info["duration"]]
            normal_wave = self.normalize_price(
                price)
            weighted_wave = np.multiply(normal_wave, wave_info['weight'])
            print(wave_info['weight'])
            # print(len(price_list), len(normal_wave))
...
for i in range(len(file_names)):
	...
	if i == 1:
		plt.figure()
		plt.plot(base_price[0:length[i]])
		plt.grid(True)
		plt.show()

	print(len(wave_2['price_list'][wave_2["start_point"]:wave_2["start_point"]+wave_2["duration"]]))
	print(name)


	Combinator = WaveModifier()
	combinated_price = Combinator.price_wave_addition(
		1, 1, length[i], wave_1, wave_2)
	...
	if i == 1:
		plt.figure()
		plt.plot(combinated_price[0:length[i]])
		plt.grid(True)
		plt.show()
	...
	for index in range(len(final_price_list)):
		random_modifier = np.random.normal(0, 0.1)
