...
for i in range(len(file_names)):
	...
	print(f"length: {len(base_price)}")
	print(len(macro["mu_sde"]))

	wave_1 = {
		'price_list': base_price,
		'start_point': 0,
		'duration': length[i],
		'weight': get_parameters.fund_weights,
		'intensity_factor': 1
	}
	...
	wave_2 = {
		...
		'duration': length[i],
		'weight': 1-get_parameters.fund_weights,
		'intensity_factor': 1
	}
	...
	for index in range(len(final_price_list)):
		random_modifier = np.random.normal(0, 0.4)
