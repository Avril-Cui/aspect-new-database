...
for i in range(len(file_names)):
	...
	print(combinated_price[index]-adjust_factor)
	print(combinated_price[index]-(combinated_price[index]-adjust_factor)))

	final_price_list = []
	for index in range(len(combinated_price)):
		if index > 0:
			price_generator = StockSimulator(
				combinated_price[index]-adjust_factor, combinated_price[index], combinated_price, index, micro, index_params[name])
	...
	for index in range(len(final_price_list)):
		random_modifier = np.random.normal(0, 0.0001)
